...
// ── MODAL COMPONENTS (separate to prevent re-render lag) ─────────────────────
function MedModal({onClose, onSave, me, T, S}) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const m = await apiFetch("POST", "/health/medicines", {
        name,
        time: timeRef.current?.value || "",
        member: memberRef.current?.value || me.name,
      }, T);
      onSave(m);
  ...
  return (
    <div style={S.modal} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={S.modalBox}>
        ...
        <div style={{fontSize:11,fontWeight:600,color:"var(--text2)",marginBottom:4}}>Name *</div>
        <input ref={nameRef} style={S.inp} placeholder="e.g. Vitamin D" autoFocus/>
        <div style={{fontSize:11,fontWeight:600,color:"var(--text2)",marginBottom:4}}>Time</div>
        <input ref={timeRef} style={S.inp} placeholder="e.g. 08:00 AM"/>
        <div style={{fontSize:11,fontWeight:600,color:"var(--text2)",marginBottom:4}}>For Member</div>
        <input ref={memberRef} style={S.inp} placeholder={me.name} defaultValue={me.name}/>
...
function LogModal({onClose, onSave, me, T, S}) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const l = await apiFetch("POST", "/health/logs", {
        text,
        bloodSugar: sugarRef.current?.value || "",
        bloodPressure: bpRef.current?.value || "",
        member: me.name,
      }, T);
      onSave(l);
    } catch(e) { alert(e.message); }
  ...
  return (
    <div style={S.modal} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={S.modalBox}>
        ...
        <input ref={textRef} style={S.inp} placeholder="e.g. Feeling dizzy today" autoFocus/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          <div>
            <div style={{fontSize:11,fontWeight:600,color:"var(--text2)",marginBottom:4}}>Blood Sugar</div>
            <input ref={sugarRef} style={S.inp} type="number" placeholder="mg/dL"/>
          </div>
          <div>
            <div style={{fontSize:11,fontWeight:600,color:"var(--text2)",marginBottom:4}}>Blood Pressure</div>
            <input ref={bpRef} style={S.inp} placeholder="120/80"/>
          </div>
        </div>
        <button style={{...S.btn(),opacity:saving?0.6:1}} onClick={save} disabled={saving}>
          {saving ? "Analyzing..." : "Save & AI Analysis 🤖"}
        </button>
      </div>
    </div>
  );
...
function MemModal({onClose, onSave, me, T, S}) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const m = await apiFetch("POST", "/memories", {
        title,
        emoji,
        lockedUntil: lockRef.current?.value || "",
        locked: !!lockRef.current?.value,
      }, T);
      onSave(m);
    } catch(e) { alert(e.message); }
    finally { setSaving(false); }
  ...
  return (
    <div style={S.modal} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={S.modalBox}>
        ...
        <input ref={titleRef} style={S.inp} placeholder="e.g. Eid 2026" autoFocus/>
        <div style={{fontSize:11,fontWeight:600,color:"var(--text2)",marginBottom:6}}>Emoji</div>
        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}>
          {["⭐","🎉","🌙","❤️","🏠","📸","🎁","🌹","🍰","✈️","🕌","🤲"].map(e=>(
            <button key={e} onClick={()=>setEmoji(e)} style={{fontSize:20,background:emoji===e?"var(--card2)":"var(--card)",border:`2px solid ${emoji===e?"var(--primary)":"var(--border)"}`,borderRadius:8,padding:"3px 8px",cursor:"pointer"}}>{e}</button>
          ))}
        </div>
        <div style={{fontSize:11,fontWeight:600,color:"var(--text2)",marginBottom:4}}>Lock until year (optional)</div>
        <input ref={lockRef} style={S.inp} placeholder="e.g. 2030"/>
        <button style={{...S.btn(),opacity:saving?0.6:1}} onClick={save} disabled={saving}>
...
export default function FamilyVerse(){
  ...
  const askAI=async()=>{
    const q=aiQueryRef.current;
    if(!q.trim())return;
    setAiLoading(true);
    try{
      const d=await apiFetch("POST","/health/ai-advice",{note:q,member:me.name},T);
      setAiResponse(d.advice);
    }catch{setAiResponse("AI unavailable. Check GROQ_API_KEY in Render settings.");}
  ...
  if(!session){
    return(<div style={{...S.root,background:"linear-gradient(135deg,#1E3A5F,#0D9488)",alignItems:"center",justifyContent:"center",padding:20,overflowY:"auto"}}>
      ...
      <div style={{width:80,height:80,borderRadius:22,background:"linear-gradient(135deg,#6366F1,#8B5CF6)",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:12,boxShadow:"0 8px 30px rgba(99,102,241,.4)"}}>
  ...
  const Health=()=>(<div style={S.page}>
    ...
    <div style={{background:"linear-gradient(135deg,var(--primary),var(--primary2))",borderRadius:16,padding:16,color:"white",marginBottom:10}}>
      ...
      <div style={{display:"flex",gap:8}}>
        <input
          defaultValue=""
          onChange={e=>{aiQueryRef.current=e.target.value;}}
    ...
    {showAddMed&&<MedModal onClose={()=>setShowAddMed(false)} onSave={m=>{setMedicines(p=>[...p,m]);setShowAddMed(false);}} me={me} T={T} S={S}/>}
    {showAddLog&&<LogModal onClose={()=>setShowAddLog(false)} onSave={l=>{setHealthLogs(p=>[l,...p]);setShowAddLog(false);}} me={me} T={T} S={S}/>}
  </div>);
...
const Faith=()=>{
  ...
  return(<div style={S.root}>
    ...
    {/* Topbar */}
    <div style={S.topbar}>
      <div style={{width:36,height:36,borderRadius:10,background:"linear-gradient(135deg,#6366F1,#8B5CF6)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:"0 2px 8px rgba(99,102,241,.4)"}}>
