...
const API = "https://familyverse-production.up.railway.app";
...
export default function FamilyVerse(){
  ...
  useEffect(()=>{if(openChat)msgsEndRef.current?.scrollIntoView({behavior:"smooth"});},[messages.length,openChat]);
  ...
  const loadQibla=useCallback(()=>{
    ...
    navigator.geolocation.getCurrentPosition(async(pos)=>{try{const r=await fetch(`https://api.aladhan.com/v1/qibla/${pos.coords.latitude}/${pos.coords.longitude}`);const d=await r.json();if(d.code===200)setQiblaDir(Math.round(d.data.direction));else setQiblaDir(290);}catch{setQiblaDir(290);}},()=>setQiblaDir(290));
  },[]);
  ...
  const connectSocket=useCallback(async(token)=>{
    ...
    try{
      ...
      socket.on("sosReceived",({from})=>alert(`🚨 SOS ALERT!\n\n${from} needs urgent help!`));
  ...
  const loadMessages=useCallback(async(chatId,token)=>{
    try{const msgs=await apiFetch("GET",`/messages/${chatId}`,null,token);setMessages(msgs||[]);}catch{}
  },[]);
  ...
  const theme={"--bg":dark?"#0f0f17":"#F8FAFC","--card":dark?"#1a1a2e":"#FFFFFF","--card2":dark?"#16213e":"#F1F5F9","--border":dark?"#2a2a4a":"#E2E8F0","--text":dark?"#E2E8F0":"#0F172A","--text2":dark?"#94A3B8":"#475569","--muted":dark?"#64748B":"#94A3B8","--primary":"#6366F1","--primary2":"#8B5CF6","--accent":"#F59E0B","--success":"#22C55E","--danger":"#EF4444","--teal":"#0D9488","--input":dark?"#1e1e2e":"#F8FAFC"};
  ...
  const unreadMsgs=messages.filter(m=>!openChat&&m.sender!==me?.name&&!m.read).length;
  ...
  const sendMsg=()=>{
    ...
    if(socketRef.current?.connected){socketRef.current.emit("sendMessage",msgData);}
    else{apiFetch("POST","/messages",msgData,T).then(msg=>setMessages(p=>[...p,msg])).catch(()=>{});}
  ...
  const askAI=async()=>{
    ...
    setAiLoading(true);
    try{const d=await apiFetch("POST","/health/ai-advice",{note:aiQuery,member:me.name},T);setAiResponse(d.advice);}
    catch{setAiResponse("AI unavailable. Check ANTHROPIC_API_KEY in Railway.");}
  ...
  const S={
    ...
    nb:(active)=>({flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2,border:"none",background:"none",cursor:"pointer",fontSize:9,fontWeight:700,color:active?"var(--primary)":"var(--muted)",padding:"4px 2px",position:"relative",fontFamily:"inherit"}),
  ...
  if(page==="chat"&&openChat){
    ...
    return(<div style={S.root}>
      ...
      <div style={{height:56,minHeight:56,display:"flex",alignItems:"center",gap:10,padding:"0 14px",background:"var(--primary)",color:"white",flexShrink:0}}>
        <button onClick={()=>{setOpenChat(null);loadMessages("group",T);}} style={{background:"none",border:"none",color:"white",fontSize:22,cursor:"pointer",padding:"0 6px 0 0",lineHeight:1}}>←</button>
        <div style={{fontSize:26,lineHeight:1}}>{openChat.avatar||openChat.emoji||"👥"}</div>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontWeight:700,fontSize:14}}>{openChat.name}</div>
          <div style={{fontSize:10,opacity:.7}}>{openChat.id==="group"?`${members.length} members`:openChat.online?"● Online":"○ Offline"} {socketConnected?"🟢":""}</div>
        </div>
      </div>
      <div style={{flex:1,overflowY:"auto",padding:12,display:"flex",flexDirection:"column",gap:8,background:"var(--bg)",WebkitOverflowScrolling:"touch"}}>
        {chatMsgs.length===0&&<div style={{textAlign:"center",padding:"40px 0",color:"var(--muted)"}}><div style={{fontSize:40,marginBottom:8}}>{openChat.avatar||"💬"}</div><div style={{fontSize:13}}>Start the conversation!</div></div>}
        {chatMsgs.map((msg,i)=>{
        ...
        {typingUser&&typingUser!==me.name&&<div style={{fontSize:11,color:"var(--muted)",fontStyle:"italic"}}>{typingUser} is typing...</div>}
        <div ref={msgsEndRef}/>
      </div>
      {showEmoji&&<div style={{padding:"8px 12px",background:"var(--card)",display:"flex",flexWrap:"wrap",gap:8,borderTop:"1px solid var(--border)"}}>{EMOJIS.map(e=><span key={e} style={{fontSize:22,cursor:"pointer"}} onClick={()=>setChatInput(p=>p+e)}>{e}</span>)}</div>}
      <div style={{flexShrink:0,background:"var(--card)",borderTop:"1px solid var(--border)",padding:"8px 12px",display:"flex",alignItems:"flex-end",gap:8}}>
        <button style={{background:"none",border:"none",fontSize:22,cursor:"pointer",lineHeight:1,color:"var(--muted)"}} onClick={()=>setShowEmoji(e=>!e)}>😊</button>
        <textarea value={chatInput} onChange={e=>handleTyping(e.target.value)} placeholder="Type a message..." rows={1}
          style={{flex:1,padding:"10px 14px",borderRadius:22,border:"1.5px solid var(--border)",background:"var(--input)",color:"var(--text)",fontFamily:"inherit",fontSize:14,outline:"none",resize:"none",minHeight:42,maxHeight:100,lineHeight:1.4,overflowY:"auto"}}
  ...
  const Dashboard=()=>(<div style={S.page}>
    ...
    <div style={S.card}>
      ...
      <div style={{display:"flex",gap:12,overflowX:"auto",paddingBottom:4}}>
        {members.map(m=>(
          <div key={m._id||m.id} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4,flexShrink:0}}>
    ...
    <div style={S.card}>
      ...
      <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:4}}>
        {MOODS.map(m=>(
          <div key={m.id} onClick={()=>{setMyMood(m.id);socketRef.current?.emit("moodUpdate",{mood:m.id});apiFetch("POST","/family/mood",{mood:m.id},T).catch(()=>{});}}
    ...
    {prayerTimes.length>0&&<div style={S.card}>
      ...
      {prayerTimes.map(p=>(
        <div key={p.name} style={{display:"flex",alignItems:"center",gap:10,padding:"7px 10px",borderRadius:10,marginBottom:5,background:nextPrayer?.name===p.name?"var(--primary)":p.passed?"var(--card2)":"var(--card)",border:nextPrayer?.name===p.name?"none":"1px solid var(--border)"}}>
    ...
    {challenges.map(c=>(
      <div key={c.id} onClick={()=>setChallenges(p=>p.map(x=>x.id===c.id?{...x,done:!x.done}:x))}
  ...
  const Health=()=>(<div style={S.page}>
    <div style={{background:"linear-gradient(135deg,var(--primary),var(--primary2))",borderRadius:16,padding:16,color:"white",marginBottom:10}}>
      <div style={{fontSize:15,fontWeight:800,marginBottom:6}}>🤖 AI Health Assistant</div>
      <div style={{display:"flex",gap:8}}>
        <input value={aiQuery} onChange={e=>setAiQuery(e.target.value)} placeholder="Ask about health..." onKeyDown={e=>e.key==="Enter"&&askAI()}
          style={{flex:1,padding:"10px 13px",borderRadius:10,border:"none",background:"rgba(255,255,255,.15)",color:"white",fontFamily:"inherit",fontSize:13,outline:"none"}}/>
        <button onClick={askAI} disabled={aiLoading} style={{padding:"10px 14px",borderRadius:10,border:"none",background:"#F59E0B",color:"white",fontFamily:"inherit",fontWeight:700,fontSize:12,cursor:"pointer",flexShrink:0}}>{aiLoading?"...":"Ask 🤖"}</button>
      </div>
      {aiResponse&&<div style={{background:"rgba(255,255,255,.12)",borderRadius:10,padding:12,marginTop:10,fontSize:12,lineHeight:1.7}}>🤖 {aiResponse}</div>}
    </div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
    ...
    <div style={S.card}>
      ...
      healthLogs.slice(0,5).map(l=>(
        <div key={l._id} style={{padding:"8px 0",borderBottom:"1px solid var(--border)"}}>
    ...
    {showAddMed&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:500,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={e=>e.target===e.currentTarget&&setShowAddMed(false)}>
      <div style={{background:"var(--card)",borderRadius:"18px 18px 0 0",padding:20,width:"100%",maxWidth:480}}>
        <div style={{fontSize:16,fontWeight:800,color:"var(--text)",marginBottom:14}}>💊 Add Medicine</div>
        {[["Name *","name","e.g. Vitamin D"],["Time","time","e.g. 08:00 AM"],["Member","member",me.name]].map(([l,k,p])=>(<div key={k}><div style={{fontSize:11,fontWeight:600,color:"var(--text2)",marginBottom:4}}>{l}</div><input style={S.inp} placeholder={p} value={fmMed[k]} onChange={e=>setFmMed({...fmMed,[k]:e.target.value})}/></div>))}
    ...
    {showAddLog&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:500,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={e=>e.target===e.currentTarget&&setShowAddLog(false)}>
      <div style={{background:"var(--card)",borderRadius:"18px 18px 0 0",padding:20,width:"100%",maxWidth:480}}>
    ...
    return(<div style={S.page}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
        ...
        <div style={S.card}>
          ...
          <div style={{textAlign:"center"}}>
            <div style={{width:110,height:110,borderRadius:"50%",background:"linear-gradient(135deg,var(--primary),var(--primary2))",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto",boxShadow:"0 6px 20px rgba(99,102,241,.3)",position:"relative"}}>
              <div style={{position:"absolute",inset:8,borderRadius:"50%",border:"1px solid rgba(255,255,255,.2)"}}/>
              <div style={{fontSize:30,transition:"transform .3s",transform:`rotate(${qiblaDir||0}deg)`}}>🕋</div>
            </div>
            <div style={{fontSize:10,color:"var(--muted)",marginTop:8}}>{qiblaDir!==null?`${qiblaDir}° · Makkah`:"Getting location..."}</div>
            {!qiblaDir&&<button style={{...S.btn(),marginTop:8,width:"auto",padding:"6px 14px",fontSize:11}} onClick={loadQibla}>📍 GPS</button>}
          </div>
        </div>
      </div>
      <div style={S.card}>
  ...
  const Kids=()=>(<div style={S.page}>
    ...
    <div style={{display:"flex",gap:6,marginBottom:10,overflowX:"auto"}}>
      {[{id:"abc",l:"🔤 ABCs"},{id:"math",l:"🔢 Math"},{id:"draw",l:"🎨 Draw"},{id:"stories",l:"📖 Stories"}].map(t=>(
        <button key={t.id} onClick={()=>setKidsTab(t.id)} style={{padding:"8px 16px",borderRadius:25,border:"none",fontFamily:"inherit",fontWeight:700,fontSize:12,cursor:"pointer",whiteSpace:"nowrap",background:kidsTab===t.id?"var(--primary)":"var(--card)",color:kidsTab===t.id?"white":"var(--text)",boxShadow:"0 2px 8px rgba(0,0,0,.07)"}}>{t.l}</button>
      ))}
    </div>
    {kidsTab==="abc"&&<div style={S.card}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:5}}>
        {Array.from("ABCDEFGHIJKLMNOPQRSTUVWXYZ").map((l,i)=>{
          ...
          return<button key={l} onClick={()=>alert(`${l} is for ${words[l]||l+"!"}`)} style={{aspectRatio:1,borderRadius:10,background:"var(--card2)",border:`2px solid ${c}44`,fontWeight:800,fontSize:15,cursor:"pointer",color:c,fontFamily:"inherit"}}>{l}</button>;
        })}
      </div>
    </div>}
    ...
    {kidsTab==="stories"&&[{t:"Prophet Ibrahim ﷺ",e:"🌟",d:"The brave prophet who built the Kaaba. A story of courage and faith."},{t:"The Spider Web",e:"🕷️",d:"How Allah used a tiny spider to protect His beloved Prophet ﷺ."},{t:"Bilal & the Adhan",e:"🕌",d:"The first muezzin whose beautiful call echoes through history."},{t:"Prophet Yusuf ﷺ",e:"👑",d:"A story of patience, forgiveness and trust in Allah's plan."}].map((s,i)=>(
      <div key={i} onClick={()=>alert(`📖 "${s.t}"\n\n${s.d}\n\nFull stories coming soon! 🌟`)} style={{...S.card,cursor:"pointer",marginBottom:8}}>
  ...
  const ChatList=()=>{
    const contacts=[{id:"group",name:family.familyName,avatar:"👨‍👩‍👧‍👦",preview:messages[messages.length-1]?.text?.substring(0,35)||"Say salaam!",online:true},...members.filter(m=>m.name!==me.name).map(m=>({id:m._id||m.id,name:m.name,avatar:m.emoji,preview:"Tap to chat...",online:m.online}))];
    return(<div style={S.page}>
      <div style={{background:"#FEF3C7",borderRadius:12,padding:"10px 14px",marginBottom:12,fontSize:12,color:"#92400E",border:"1px solid #FCD34D"}}>
        🔑 Invite: <strong style={{letterSpacing:1}}>{family.inviteCode}</strong>
      </div>
      {contacts.map(c=>(<div key={c.id} onClick={()=>{setOpenChat(c);loadMessages(c.id==="group"?"group":[me.name,c.name].sort().join("-"),T);}} style={{...S.card,display:"flex",alignItems:"center",gap:12,cursor:"pointer",marginBottom:8}}>
        ...
        {c.id==="group"&&unreadMsgs>0&&<div style={{background:"var(--danger)",color:"white",fontSize:10,fontWeight:800,width:20,height:20,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{unreadMsgs}</div>}
      </div>))}
    </div>);
  };
  ...
  const Memories=()=>(<div style={S.page}>
    ...
    {showAddMem&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:500,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={e=>e.target===e.currentTarget&&setShowAddMem(false)}>
      <div style={{background:"var(--card)",borderRadius:"18px 18px 0 0",padding:20,width:"100%",maxWidth:480}}>
        ...
        <div style={{fontSize:11,fontWeight:600,color:"var(--text2)",marginBottom:4}}>Lock until year</div>
        <input style={S.inp} placeholder="e.g. 2030 (optional)" value={fmMem.lockedUntil} onChange={e=>setFmMem({...fmMem,lockedUntil:e.target.value})}/>
        <button style={S.btn()} onClick={()=>{if(!fmMem.title.trim())return alert("Enter title!");apiFetch("POST","/memories",{...fmMem,locked:!!fmMem.lockedUntil},T).then(m=>{setMemories(p=>[m,...p]);setFmMem({title:"",emoji:"⭐",lockedUntil:""});setShowAddMem(false);}).catch(e=>alert(e.message));}}>Save 💾</button>
      </div>
    </div>}
  </div>);

  const Emergency=()=>(<div style={{...S.page,textAlign:"center"}}>
    {sosActive&&<div style={{background:"#FEE2E2",border:"2px solid #FCA5A5",borderRadius:14,padding:14,marginBottom:14,textAlign:"left"}}>
    ...
    <div style={{padding:"20px 0 16px"}}>
      ...
      <div style={{fontSize:12,color:"var(--muted)",maxWidth:240,margin:"0 auto"}}>Instantly alerts all family members.</div>
    </div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
      {[{e:"📍",t:"Share Location",d:"GPS"},{e:"📞",t:"Emergency",d:"Call 112"},{e:"🏥",t:"Hospital",d:"Find nearest"},{e:"👮",t:"Police",d:"Authorities"}].map(a=>(
        <div key={a.t} style={{...S.card,cursor:"pointer",padding:"16px 10px"}} onClick={()=>alert(`${a.e} ${a.t}`)}>
  ...
  const Settings=()=>(<div style={S.page}>
    ...
    <div style={{...S.card}}>
      ...
      <div style={{background:"#FEF3C7",borderRadius:10,padding:"10px 12px"}}>
        <div style={{fontSize:10,fontWeight:700,color:"#92400E",marginBottom:3}}>🔑 INVITE CODE</div>
        <div style={{fontSize:20,fontWeight:900,color:"#F59E0B",letterSpacing:2,fontFamily:"monospace"}}>{family.inviteCode}</div>
        <div style={{fontSize:10,color:"#92400E",marginTop:4}}>Share → Join Existing Family → Enter code ✅</div>
      </div>
    </div>
  ...
  return(<div style={S.root}>
    ...
    <div style={{height:socketConnected?0:22,overflow:"hidden",background:"#EF4444",color:"white",textAlign:"center",fontSize:10,fontWeight:700,lineHeight:"22px",flexShrink:0,transition:"height .3s"}}>🔴 Connecting...</div>
    <div style={S.topbar}>
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        <div style={{fontSize:9,color:"var(--muted)"}}>{page.charAt(0).toUpperCase()+page.slice(1)}</div>
      </div>
    ...
    <div style={S.scroll}>{PAGES[page]}</div>
    <div style={S.bnav}>
      {NAV.map(n=>(<button key={n.id} style={S.nb(page===n.id)} onClick={()=>{setPage(n.id);setOpenChat(null);}}>
